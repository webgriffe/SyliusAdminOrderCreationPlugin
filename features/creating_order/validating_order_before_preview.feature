@admin_order_creation_managing_orders @ui @javascript
Feature: Validating the order before previewing it
    In order to avoid placing an incomplete order
    As an Administrator
    I want the order creation form to be validated before I see its preview

    Background:
        Given the store operates on a single channel in "United States"
        And the store has a product "Stark Coat" priced at "$100.00"
        And the store ships everywhere for free
        And the store allows paying with "Cash on Delivery"
        And there is a customer account "jon.snow@the-wall.com"
        And I am logged in as an administrator

    Scenario: Trying to preview an order without a shipping address
        When I create a new order for "jon.snow@the-wall.com" and channel "United States"
        And I add "Stark Coat" to this order
        And I place this order
        Then I should still be on the order creation form
        And I should see a validation error
